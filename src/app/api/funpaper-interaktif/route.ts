...
export async function GET(req: Request) {
  try {
    ...
    if (temaArr.length === 0 || temaArr.includes(FUNPAPER_CALISTUNG)) {
      const sqlCalistung = `
    ...
    if (temaArr.length === 0 || temaArr.includes(FUNPAPER_CODING)) {
      const sqlCoding = `
      ...
      itemsCoding = (dataCoding?.result?.[0]?.results ?? []).map(
        (item: any) => ({
          ...item,
          category: "coding",
    ...
    if (temaArr.length === 0 || temaArr.includes(FUNPAPER_HARIAN)) {
      const sqlHarian = `
      ...
      itemsHarian = (dataHarian?.result?.[0]?.results ?? []).map(
        (item: any) => ({
          ...item,
          category: "harian",
