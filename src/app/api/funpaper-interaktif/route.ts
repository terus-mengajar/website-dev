...
export async function GET(req: Request) {
  try {
    ...
    const sqlCalistung = `
      SELECT funpaper_calistung.downloaded, funpaper_calistung.played, funpaper_calistung.slug, funpaper_calistung.updated_at, funpaper_calistung.image_url, CONCAT(funpaper_calistung.name, ' - ', theme_calistung.name) AS name
    ...
    const itemsCalistung = (dataCalistung?.result?.[0]?.results ?? []).map((item: any) => ({
      ...item,
      category: "calistung",
    ...
    const sqlCoding = `
      SELECT funpaper_coding.downloaded, funpaper_coding.played, funpaper_coding.slug, funpaper_coding.updated_at, funpaper_coding.image_url, funpaper_coding.name
    ...
    const itemsCoding = (dataCoding?.result?.[0]?.results ?? []).map((item: any) => ({
      ...item,
      category: "coding",
    ...
    const sqlHarian = `
      SELECT funpaper.downloaded, funpaper.played, funpaper.slug, funpaper.updated_at, funpaper.image_url, CONCAT(funpaper.name, ' - ', activity.name) AS name
      FROM funpaper
    ...
    const itemsHarian = (dataHarian?.result?.[0]?.results ?? []).map((item: any) => ({
      ...item,
      category: "harian",
