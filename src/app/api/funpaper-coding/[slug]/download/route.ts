...
export async function POST(request: Request) {
  try {
    const { email, funpaper_coding_id } = await request.json();
    ...
    const updateRes = await fetch(CLOUDFLARE_D1_URL, {
      ...
      body: JSON.stringify({
        sql: `
            UPDATE funpaper_coding
            SET downloaded = downloaded + 1
            WHERE id = ?
          `,
        params: [funpaper_coding_id],
      }),
    });
    ...
    const insertRes = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${process.env.CLOUDFLARE_ACCOUNT_ID}/d1/database/${process.env.CLOUDFLARE_DATABASE_ID}/query`,
      {
        ...
        body: JSON.stringify({
          sql: `
            INSERT INTO funpaper_coding_download_log (created_at, email, funpaper_coding_id)
            VALUES (CURRENT_TIMESTAMP, ?, ?)
          `,
          params: [email, funpaper_coding_id],
        }),
      },
    );
