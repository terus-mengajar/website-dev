...
export async function generateMetadata({ params, searchParams }) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/funpaper-interaktif/${slug}?tipe=${tipe}`,
    {
...
export default async function FunpaperPage({ params, searchParams }) {
  const { slug } = await params;
  const { tipe } = await searchParams;
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/funpaper-interaktif/${slug}?tipe=${tipe}`,
