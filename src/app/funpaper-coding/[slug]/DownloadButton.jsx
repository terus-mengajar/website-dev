...
export default function FunpaperDownload({ id, slug, link, interactive }) {
  ...
  const handleDownload = async () => {
    if (status !== "authenticated") {
      ...
      router.push(`/auth/login?callbackUrl=/funpaper-coding/${slug}`);
      return;
    }
    ...
    try {
      ...
      await fetch(`/api/funpaper-coding/${slug}/download`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          funpaper_coding_id: id, // isi dari props atau data
        }),
      });
    } catch (err) {
